...
class IntraModalCompactLoss(nn.Module):
    ...
    def _proto_supcon_ce(self, feat, label):
        ...
        # 统计同模态原型
        proto, ids, sum_g, cnt_g = self._proto_stats(feat, label)   #求中心
        ...
        return F.cross_entropy(logits[has_pos], target_idx[has_pos])        #和除自己之外的类别

    def _center_var(self, feat, label):
        """
        样本到“自身ID原型(去自身) ”的 SmoothL1（proto stop-grad）
        """
        ...
        with torch.no_grad():
            target = proto_excl  # stop-grad

        diff = F.smooth_l1_loss(feat[has_pos], target[has_pos], reduction='none').sum(dim=1)
        return diff.mean()

    def _center_separation(self, feat, label):
        """
        用同模态原型做分离：对不同ID原型，若 cos > m_sep，惩罚 (cos - m_sep)+
        可选只取 top-k 最相近的原型对以避免过强约束。
        """
        ids = torch.unique(label)
        if ids.numel() <= 1:
            return feat.new_tensor(0.0)

        proto, ids, _, _ = self._proto_stats(feat, label)
        if self.normalize:
            proto = F.normalize(proto, dim=1)
        S = proto @ proto.t()
        K = S.size(0)
        off = ~torch.eye(K, dtype=torch.bool, device=feat.device)
        viol = S[off] - self.m_sep
        viol = F.relu(viol)

        if self.topk_sep is not None and viol.numel() > 0:
            # 仅保留最相近的 top-k 对
            k = min(self.topk_sep, viol.numel())
            topk_vals, _ = torch.topk(viol.view(-1), k, largest=True)
            return topk_vals.mean()
        return viol.mean() if viol.numel() > 0 else feat.new_tensor(0.0)
    ...
    def forward(self, feats, labels, parts=None):
        ...
        L_proto_nce = 0.5 * (L_nce_vis + L_nce_ir)          #去自身求相似度

        # (2) 方差收缩（同模态到自身ID原型，SmoothL1）
        L_var = 0.5 * (self._center_var(f_vis, y_vis) + self._center_var(f_ir, y_ir))    #保持类间距离，惩罚机制

        # (3) 原型分离（同模态）
        L_sep = 0.5 * (self._center_separation(f_vis, y_vis) +
                       self._center_separation(f_ir , y_ir))
        ...
        if parts and self.lambda_local > 0:
            ...
            for p in parts:
                p = p.float()
                pv, pi = p[:b2], p[b2:]
                acc += 0.5 * (self._center_var(pv, y_vis) + self._center_var(pi, y_ir))
                n += 1
            if n > 0: L_local = acc / n

        loss = L_proto_nce + self.lambda_var * L_var + self.lambda_sep * L_sep + self.lambda_local * L_local
        stat = {
