...
parser.add_argument('--tvsearch',default=0, action='store_true', help='whether thermal to visible search on RegDB')
...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            feat_fc = net_vit(inputO, inputO, test_mode[1], seq_len=1)
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            batch_num = input.size(0)
            inputO = Variable(input.cuda())
            feat_fc = net_vit(inputO, inputO, test_mode[0], seq_len=1)
            feat_fc=feat_fc
...
if dataset == 'sysu':
    ...
    if len(args.resume) > 0:
        ...
        print(model_path)
        model_path = checkpoint_path + 'sysu_LAReViT_p3_n8_lr_0.0003_seed_0_ADMW_best.t'
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path,weights_only=False,map_location={
    ...
    for trial in range(10):
        ...
        model_path = checkpoint_path + 'regdb_LAReViT_p4_n6_lr_0.0003_seed_0_sgd_trial_1_best.t'.format(test_trial)
        ...
        if os.path.isfile(model_path):
            ...
            print('==> no_loading checkpoint {}'.format(model_path))
    ...
    if len(args.resume) > 0:
        model_path = checkpoint_path
        print(model_path)
        model_path = checkpoint_path + 'v-illcm_LAReViT_p4_n6_lr_0.0003_seed_0_ADMW_best.t'
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path, weights_only=False, map_location={
                'cuda:2': 'cuda:0', 'cuda:1': 'cuda:0'})
