...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        # ---- 安全插值门：β∈[0,1]，初始=0 → 完全等同 baseline 自注意力 ----
        # 用 logit 形式存储，初始化为 0.0 → sigmoid(0)=0.5，再乘一个 scale 让初值严格为 0
        # 这里更直接：把 beta 作为 nn.Parameter(0.0)，再经 softplus 压到 [0,∞)，映射到 [0,1)
        self.beta_raw = nn.Parameter(torch.tensor(0.0))  # 初始0，保证不偏离baseline

        # ---- LayerScale（设为1.0保证与原版量级一致）----
        self.gamma1 = nn.Parameter(torch.ones(dim))  # attn 残差缩放
        self.gamma2 = nn.Parameter(torch.ones(dim))  # mlp 残差缩放

    @staticmethod
    def _squash01(x):
        # 把任意实数单标量映射到 [0,1)；初值0→0
        # 1 - exp(-softplus(x)) 既稳定又有梯度
        return 1.0 - torch.exp(-torch.nn.functional.softplus(x))

    def forward(self, x, x_ori):
        """
        x:     (B, N, C)
        x_ori: (B, N, C)  # 参考/跨支路上下文（你的原始代码已传入）
        """
        nx    = self.norm1(x)
        n_ori = self.norm1(x_ori)

        # baseline 自注意力
        attn_self  = self.attn(nx, nx)
        # cross 注意力
        attn_cross = self.attn(nx, n_ori)

        # 安全插值：beta 初始为 0 → 完全退化成 baseline
        beta = self._squash01(self.beta_raw)        # 标量 ∈ [0,1)
        attn_out = attn_self + beta * (attn_cross - attn_self)

        x = x + self.drop_path(attn_out * self.gamma1)
        x = x + self.drop_path(self.mlp(self.norm2(x)) * self.gamma2)
        return x
...
class ViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x_ori03=x_3
        x   = self.blocks[0](x  ,x  )
        # 如果 0 在 deep_layers，就收集
        # if 0 in self.deep_layers:
        #     cls_list.append(self.cls_norm(x)[:, 0])  # (B, C)
        ...
        # if 1 in self.deep_layers:
        #     cls_list.append(self.cls_norm(x)[:, 0])
